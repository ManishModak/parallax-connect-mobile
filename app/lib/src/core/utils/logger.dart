...
class MinimalPrinter extends LogPrinter {
  ...
  String _getLevelTag(Level level) {
    switch (level) {
      ...
      case Level.info:
        return colors ? '${LogColors.brightGreen}[INF]${LogColors.reset}' : '[INF]';
      case Level.warning:
        return colors ? '${LogColors.brightYellow}[WRN]${LogColors.reset}' : '[WRN]';
      case Level.error:
        return colors ? '${LogColors.brightRed}[ERR]${LogColors.reset}' : '[ERR]';
      case Level.fatal:
  ...
  List<String> log(LogEvent event) {
    final lines = <String>[];
    final time = printTime ? '${LogColors.gray}${_getTime()}${LogColors.reset} ' : '';
...
class FileOutput extends LogOutput {
  ...
  Future<void> _initLogFile() async {
    try {
      ...
      // Use project root applogs folder for development
      final logDir = Directory('applogs');
  ...
  Future<void> _cleanupOldLogs(Directory logDir, {int keepCount = 5}) async {
    try {
      ...
      for (final file in toDelete) {
        await file.delete();
        debugPrint('🗑️ Deleted old log: ${file.path}');
      }
    } catch (e) {
...
class Log {
  ...
  static void network(String message, [dynamic data]) {
    logger.i('${LogColors.brightBlue}🌐 NET${LogColors.reset} $message${data != null ? ': $data' : ''}');
  }

  /// Navigation events
  static void nav(String message, [dynamic data]) {
    logger.d('${LogColors.magenta}🧭 NAV${LogColors.reset} $message${data != null ? ': $data' : ''}');
  }

  /// Storage operations
  static void storage(String message, [dynamic data]) {
    logger.d('${LogColors.green}💾 DB${LogColors.reset} $message${data != null ? ': $data' : ''}');
  }
  ...
  static void api(String method, String url, {int? status, dynamic body}) {
    final statusColor = status != null && status >= 200 && status < 300
        ...
        : LogColors.brightRed;
    final statusText = status != null ? ' $statusColor[$status]${LogColors.reset}' : '';
    logger.i('${LogColors.brightCyan}📡 API${LogColors.reset} $method $url$statusText');
    if (body != null) {
      logger.d('    ↳ $body');
    }
  }
  ...
  static void action(String message, [dynamic data]) {
    logger.d('${LogColors.yellow}👆 ACT${LogColors.reset} $message${data != null ? ': $data' : ''}');
  }
}
