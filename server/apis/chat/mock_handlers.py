...
async def mock_stream(request: ChatRequest):
    ...
    # 1. Analyze Intent & Check for Client-Side Search Results
    if "[WEB SEARCH RESULTS]" in request.prompt:
        logger.info(f"🔍 [{request_id}] [MOCK] Detected client-side search results")
        yield f"data: {json.dumps({'type': 'thinking', 'content': 'Reading provided search results...'})}\n\n"
        await asyncio.sleep(1.0)

        yield f"data: {json.dumps({'type': 'thinking', 'content': 'Synthesizing answer from search data...'})}\n\n"
        await asyncio.sleep(1.0)

        # Extract query if possible (heuristic)
        search_query = "your query"
        match = re.search(r"User Question: (.*)", request.prompt, re.DOTALL)
        if match:
            search_query = match.group(1).strip()

        response_text = (
            f"### Analysis of '{search_query}'\n\n"
            "Based on the search results you provided, here is a summary:\n\n"
            "*(Mocking intelligent synthesis of the provided context...)*\n\n"
            "The search results indicate significant activity regarding this topic. "
            "Several sources confirm the key details you are looking for.\n\n"
            "**Key Takeaways:**\n"
            "- The data suggests a strong upward trend.\n"
            "- Experts are divided but generally optimistic.\n"
            "- Recent events have accelerated this shift.\n\n"
            "*(Generated by Parallax Mock Server using Client-Side Context)*"
        )
        ...
        yield f"data: {json.dumps({'type': 'done', 'metadata': {'prompt_tokens': 100, 'completion_tokens': completion_tokens, 'model': 'mock-model', 'duration_seconds': round(elapsed, 2)}})}\n\n"
        return
    ...
    if len(request.prompt.split()) < 2 and q_lower in ["hi", "hello", "test"]:
        ...
        # Always trigger search if explicitly enabled, regardless of keywords
